...
class deletePhoto extends Component {
    ...
    render() {
        ...
        let images = this.state.photos.map(image => {
          i = i+1 
          return <Card.Grid key={i} style={{width:'20%',height:250,textAlign:'center'}}>
            <Row>
            <img alt="example" style={{width:150,maxHeight:200}} src={image.src}/>
            </Row>
            <Row>
            <Button type="link" onClick={()=>
               MySwal.fire({
                title: 'Are you sure to delete Photo?',
                text: "Yes to Delete Photo in Storage!",
                type: 'warning',
